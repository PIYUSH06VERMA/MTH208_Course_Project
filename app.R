...
ui<- page_navbar(
  ...
  #introduction statements to be added by me
  # TAB 1: INTRODUCTION
  # -------------------------------------------------------------------
  tabPanel(
    title = "Introduction",
      ...
      card(
        ...
        tabsetPanel(
          ...
          tabPanel(
            title="welcome",
            br(),
            h4("Click one of the cards above to explore a plot!", 
               style = "text-align:center; color:grey; font-style:italic;"),
            br()
          ),
          tabPanel(
            title="Preston Curve",
            h4("Preston Curve: Life Expectancy vs. GDP per Capita"),
            p("This plot shows the relationship between national income and life expectancy."),
            plotOutput("preston_plot")
          )
        )
  ),
...
server <- function(input, output,session) {
  observeEvent(input$goto_preston, {
    updateTabsetPanel(session, "plot_tabs", selected = "Preston Curve")
  })
  output$preston_plot <- renderPlot({
    
    plot(mtcars$wt, mtcars$mpg, 
         main = "Placeholder Plot (Replace Me)",
         xlab = "wt", ylab = "mpg", col = "darkgreen", pch = 16)
